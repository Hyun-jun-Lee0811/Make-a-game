...
{
	if (objectB->GetGOComponent<RectCollision>() != nullptr)
	{
		if (GetWorldCoorRect().Right() > objectB->GetGOComponent<RectCollision>()->GetWorldCoorRect().Left() && GetWorldCoorRect().Left() < objectB->GetGOComponent<RectCollision>()->GetWorldCoorRect().Right() &&
...
{
	if (point.y >= GetWorldCoorRect().Bottom() && point.x <= GetWorldCoorRect().Right()
		&& point.y <= GetWorldCoorRect().Top() && point.x >= GetWorldCoorRect().Left())
	{
		return true;
	}
	return false;
...
{
	if (objectB->GetGOComponent<CircleCollision>() != nullptr)
	{
		if ((objectPtr->GetPosition().x - objectB->GetPosition().x) *
			(objectPtr->GetPosition().x - objectB->GetPosition().x) +
			(objectPtr->GetPosition().y - objectB->GetPosition().y)
			* (objectPtr->GetPosition().y - objectB->GetPosition().y)
			< (GetRadius() + objectB->GetGOComponent<CircleCollision>()->GetRadius())
			* (GetRadius() + objectB->GetGOComponent<CircleCollision>()->GetRadius()))
		{
			return true;
		}
...
bool CS230::CircleCollision::DoesCollideWith(math::vec2 point)
{
	if ((objectPtr->GetPosition().x - point.x) * (objectPtr->GetPosition().x - point.x)
		+ (objectPtr->GetPosition().y - point.y) * (objectPtr->GetPosition().y - point.y) < (GetRadius() * GetRadius()))
	{
		return true;
	}
	return false;
